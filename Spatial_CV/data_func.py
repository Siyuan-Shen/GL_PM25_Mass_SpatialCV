...
def get_longterm_array(area, imonth, beginyear, endyear, final_data_recording,obs_data_recording):
    final_longterm_data = np.zeros(final_data_recording[area][endyear][imonth].shape, dtype=np.float64)
    obs_longterm_data   = np.zeros(final_data_recording[area][endyear][imonth].shape, dtype=np.float64)
    for iyear in range(endyear-beginyear+1):
